  ...
  passport.use(
    new Strategy(
      ...
      async (username, password, done) => {
        ...
        if (!user)
          return done(
            { ...unauthorizeMessage, description: "Username tidak terdaftar" },
            false
          );
        if (!(await bcrypt.compare(password, user.password)))
          return done(
            { ...unauthorizeMessage, description: "Password invalid" },
            false
          );
        done(null, user);
      }
...
export const isAuthenticated = (req, res, next) => {
  ...
  return res
    .status(401)
    .json({ ...unauthorizeMessage, description: "User is not authenticated" });
};
